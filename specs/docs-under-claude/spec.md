...
**Problème** : `docs/` est le territoire du projet utilisateur. La collision est frontale : user-project a son propre `docs/ARCHITECTURE.md` (Mermaid, infra Terraform/Proxmox) qui s'est fait écraser par celui du socle pendant l'install. Toute mise à jour du socle peut clobberer la doc du user.
...
- user-project (et autres projets infra) peuvent installer/mettre à jour le socle sans collision avec leur propre `docs/`
