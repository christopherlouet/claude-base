...
| **Scratchpad size** | SessionStart | Claude Code gives every session a temp dir (its scratchpad) and **nothing removes it when the session ends**; where `/tmp` is a tmpfs that is RAM (measured 2026-09-24: one session left 4 GB of `node_modules` there and the OOM killer ended another session). When the whole tree passes `CLAUDE_BASE_SCRATCH_WARN_MB` (default 1024) it prints the total, the three largest other sessions with their rebuildable dirs, and a tmpfs note; it always reports files you do not own (a `docker run -v` without `--user`). A scan that outlives its budget (`CLAUDE_BASE_SCRATCH_SCAN_SECONDS`, default 5, where `timeout` exists) is reported as too large to measure, never skipped. Runs on `startup` only, not on each compaction. **Reports only, never deletes.** To move the tree to disk, set `CLAUDE_CODE_TMPDIR` in `~/.claude/settings.json` `env` — project-level `env` cannot set it since Claude Code 2.1.251 (`scripts/hooks/scratchpad-check.sh`) |
| **Main protection** | PreToolUse (Edit/Write) | Keeps edits off main/master by auto-creating a `feature/auto-<timestamp>` branch first; **blocks the edit** (exit 2) if the branch can't be created, instead of letting it land on main. Scoped to the **target path**: an edit aimed outside the working tree (a dotfile in `$HOME`, another project) is left alone, so it no longer strands an empty `feature/auto-…` behind it — an unreadable target (no `jq`, no path) still guards. Disable with `ALLOW_MAIN_EDIT=1` (`scripts/hooks/main-branch-guard.sh`) |
...
| **Pre-commit tests** | PreToolUse (Bash git commit) | Runs the stack's test suite (npm/pytest/go) before a commit and **blocks (exit 2) on failure**; npm's `npm init -y` placeholder test script (always exits 1, no pretest/posttest) is skipped with a notice, not run, and the pytest / go test checks still run; detects/repairs Husky if needed. Disable with `SKIP_PRE_COMMIT_TESTS=1` (`scripts/hooks/pre-commit-tests.sh`) |
| **Local pre-push CI** | PreToolUse (Bash git push) | Lint + type-check + tests before push; npm's placeholder test script is skipped with a notice, and a package.json holding only that placeholder does not stop the Python / Go checks. Disable with `SKIP_PRE_PUSH_CI=1` |
| **Pre-deploy build** | PreToolUse (Bash deploy) | Runs the production build (`npm run build` / `go build`) before a deploy command and **blocks (exit 2) on failure** — including a broken Go build. Disable with `SKIP_PRE_DEPLOY_BUILD=1` (`scripts/hooks/pre-deploy-build.sh`) |
| **Destructive ops guard** | PreToolUse (Bash) | Blocks destructive DB/filesystem ops — DROP/TRUNCATE, an unscoped `DELETE FROM` (no WHERE), wiping an uploads/media/storage tree, `prisma migrate reset`. Disable with `SKIP_DESTRUCTIVE_CHECK=1` (`scripts/hooks/destructive-ops.sh`) |
| **Command validator** | PreToolUse (Bash) | Validates commands against 9 risk categories (fork bombs, pipe-to-shell, disk destruction, privilege escalation, deleting a whole home directory whether spelled as a path, `~` or `$HOME`, **`git --no-verify` / `commit -n` gate-bypass**, etc.). Disable all with `SKIP_COMMAND_VALIDATOR=1`, or just the no-verify block with `SKIP_NO_VERIFY_CHECK=1` |
...
| `pre-push` → **preflight** | Runs the foundation's own CI gates locally before a push (`scripts/preflight.sh --fast`: shellcheck · `validate-counts.sh` · `manifest-hooks-coverage`), so failures surface locally instead of post-push. Mirrors `ci.yml`; `scripts/preflight.sh --full` adds the complete bats suite. A gate whose tool is absent does not run: it prints `SKIPPED` on its own line and the run withholds the "all gates passed" line (non-blocking — CI's Linux job is authoritative). Bypass once with `SKIP_PREFLIGHT=1`. |
