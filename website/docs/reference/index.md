---
sidebar_position: 1
title: Reference
description: Reference rapide et matrices
---

# Reference

> Acces rapide a toutes les ressources claude-socle

## Matrices de reference

| Reference | Description |
|-----------|-------------|
| [Matrice des Commands](/docs/reference/commands-matrix) | 100 commandes avec domaine et description |
| [Matrice des Agents](/docs/reference/agents-matrix) | 37 agents avec modele et outils |
| [Cheatsheet](/docs/reference/cheatsheet) | Aide-memoire rapide |

## Navigation rapide

### Par domaine

| Domaine | Commandes | Description |
|---------|-----------|-------------|
| [WORK](/docs/commands/work) | 10 | Workflow principal |
| [DEV](/docs/commands/dev) | 16 | Developpement |
| [QA](/docs/commands/qa) | 11 | Qualite |
| [OPS](/docs/commands/ops) | 24 | Operations |
| [DOC](/docs/commands/doc) | 9 | Documentation |
| [BIZ](/docs/commands/biz) | 11 | Business |
| [GROWTH](/docs/commands/growth) | 9 | Croissance |
| [DATA](/docs/commands/data) | 3 | Donnees |
| [LEGAL](/docs/commands/legal) | 5 | Legal |

### Par composant

| Composant | Nombre | Lien |
|-----------|--------|------|
| Commands | 100 | [Voir](/docs/commands) |
| Agents | 37 | [Voir](/docs/agents) |
| Skills | 24 | [Voir](/docs/skills) |
| Rules | 15 | [Voir](/docs/rules) |

## Raccourcis

### Commandes essentielles

```bash
/assistant       # Guide complet
/work-explore    # Explorer le code
/work-plan       # Planifier
/dev-tdd         # Developper en TDD
/work-commit     # Commit propre
/work-pr         # Pull Request
/qa-audit        # Audit complet
```

### Workflows pre-definis

```bash
/work-flow-feature "..."   # Nouvelle feature
/work-flow-bugfix "..."    # Correction bug
/work-flow-release "..."   # Release
/work-flow-launch "..."    # Lancement produit
```

---

## Voir aussi

- [Workflows](/docs/workflow)
- [Guides](/docs/guides)
- [Architecture](/docs/intro/architecture)
