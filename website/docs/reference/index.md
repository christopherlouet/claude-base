---
sidebar_position: 1
title: Reference
description: Reference rapide et matrices
---

# Reference

> Acces rapide a toutes les ressources claude-socle

## Matrices de reference

| Reference | Description |
|-----------|-------------|
| [Matrice des Commands](/docs/reference/commands-matrix) | 119 commandes avec domaine et description |
| [Matrice des Agents](/docs/reference/agents-matrix) | 57 agents avec modele et outils |
| [Scripts utilitaires](/docs/reference/scripts) | 12 scripts d'installation et maintenance |
| [Cheatsheet](/docs/reference/cheatsheet) | Aide-memoire rapide |

## Navigation rapide

### Par domaine

| Domaine | Commandes | Description |
|---------|-----------|-------------|
| [WORK](/docs/commands/work) | 10 | Workflow principal |
| [DEV](/docs/commands/dev) | 16 | Developpement |
| [QA](/docs/commands/qa) | 11 | Qualite |
| [OPS](/docs/commands/ops) | 24 | Operations |
| [DOC](/docs/commands/doc) | 9 | Documentation |
| [BIZ](/docs/commands/biz) | 11 | Business |
| [GROWTH](/docs/commands/growth) | 9 | Croissance |
| [DATA](/docs/commands/data) | 3 | Donnees |
| [LEGAL](/docs/commands/legal) | 5 | Legal |

### Par composant

| Composant | Nombre | Lien |
|-----------|--------|------|
| Commands | 119 | [Voir](/docs/commands) |
| Agents | 57 | [Voir](/docs/agents) |
| Skills | 41 | [Voir](/docs/skills) |
| Rules | 20 | [Voir](/docs/rules) |
| Scripts | 12 | [Voir](/docs/reference/scripts) |

## Raccourcis

### Commandes essentielles

```bash
/assistant       # Guide complet
/work:work-explore    # Explorer le code
/work:work-plan       # Planifier
/dev:dev-tdd         # Developper en TDD
/work:work-commit     # Commit propre
/work:work-pr         # Pull Request
/qa:qa-audit        # Audit complet
```

### Workflows pre-definis

```bash
/work:work-flow-feature "..."   # Nouvelle feature
/work:work-flow-bugfix "..."    # Correction bug
/work:work-flow-release "..."   # Release
/work:work-flow-launch "..."    # Lancement produit
```

---

## Voir aussi

- [Workflows](/docs/workflow)
- [Guides](/docs/guides)
- [Architecture](/docs/intro/architecture)
