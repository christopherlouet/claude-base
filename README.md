...
[![Tests](https://img.shields.io/badge/tests-1665%20passing-brightgreen)](./tests)
...
```bash
curl -fsSL https://raw.githubusercontent.com/christopherlouet/claude-base/main/install.sh | bash -s -- --ref v5.2.0
```
...
| `tests/` | <!-- count:tests -->1665<!-- /count --> bats tests across <!-- count:testFiles -->75<!-- /count --> files |
...
<!-- count:tests -->1665<!-- /count --> bats tests across <!-- count:testFiles -->75<!-- /count --> files. A few anchors :
...
- <!-- count:tests -->1665<!-- /count --> bats tests run on every PR (Linux + macOS), parallelised via `./scripts/test.sh`
...
- Pinned versions via git tags (current : v<!-- version -->5.2.0<!-- /version -->) with full `CHANGELOG.md` in Keep-a-Changelog format
