...
[![Tests](https://img.shields.io/badge/tests-1113%20passing-brightgreen)](./tests)
...
The foundation ships <!-- count:commands -->117<!-- /count --> commands + <!-- count:agents -->47<!-- /count --> agents + <!-- count:skills -->53<!-- /count --> skills, but most skills are **thin pointers** pairing the canonical vendor source with a few foundation-specific discipline lines (security/GDPR wraps, anti-patterns, cross-skill orchestration). What's NOT pointer-shaped is the workflow layer:
...
|---|---|---|
| Slash commands | <!-- count:commands -->117<!-- /count --> across 9 domains (work, dev, qa, ops, doc, biz, growth, data, legal) | Manually triggered (`/work:work-plan`) |
| Sub-agents | <!-- count:agents -->47<!-- /count --> | Autonomous, isolated-context workers spawned by commands |
...
| `tests/` | <!-- count:tests -->1113<!-- /count --> bats tests across <!-- count:testFiles -->45<!-- /count --> files |
...
## Available Commands (<!-- count:commands -->117<!-- /count -->)
...
It covers:
- Quick start guide
- Catalog of <!-- count:commands -->117<!-- /count --> commands, <!-- count:agents -->47<!-- /count --> agents, <!-- count:skills -->53<!-- /count --> skills, <!-- count:rules -->31<!-- /count --> rules
...
<!-- count:tests -->1113<!-- /count --> bats tests across <!-- count:testFiles -->45<!-- /count --> files. A few anchors :
...
- <!-- count:tests -->1113<!-- /count --> bats tests run on every PR (Linux + macOS), parallelised via `./scripts/test.sh`
...
- Pinned versions via git tags (current : v<!-- version -->4.2.0<!-- /version -->) with full `CHANGELOG.md` in Keep-a-Changelog format
