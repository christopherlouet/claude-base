...
[![Tests](https://img.shields.io/badge/tests-1773%20passing-brightgreen)](./tests)
...
```bash
curl -fsSL https://raw.githubusercontent.com/christopherlouet/claude-base/main/install.sh | bash -s -- --ref v5.2.1
```
...
| `tests/` | <!-- count:tests -->1773<!-- /count --> bats tests across <!-- count:testFiles -->80<!-- /count --> files |
...
<!-- count:tests -->1773<!-- /count --> bats tests across <!-- count:testFiles -->80<!-- /count --> files. A few anchors :
...
`claude-base update` is COPY-only by default — existing files in your project's `.claude/` are not deleted. Pass `--all --clean` to wipe-and-replace (a full backup is created first; `--clean` alone is refused, since it would wipe every managed dir but re-copy only commands). `update` also **flags a stack pivot**: if your project has outgrown its recorded preset (e.g. a Vite SPA that grew into Next.js), it prints a non-blocking notice pointing at `claude-base update --preset <name>`; `--detect-only` reports that check (`Diverges: yes/no`) without updating anything.
...
- <!-- count:tests -->1773<!-- /count --> bats tests run on every PR (Linux + macOS), parallelised via `./scripts/test.sh`
...
- Pinned versions via git tags (current : v<!-- version -->5.2.1<!-- /version -->) with full `CHANGELOG.md` in Keep-a-Changelog format
