...
[![Tests](https://img.shields.io/badge/tests-1158%20passing-brightgreen)](./tests)
...
| Path | Role |
|---|---|
| `bin/claude-base` | CLI dispatcher (`init` / `update` / `validate` / `doctor` / `preset` / `uninstall` / `version` / `help`) |
...
| `tests/` | <!-- count:tests -->1158<!-- /count --> bats tests across <!-- count:testFiles -->45<!-- /count --> files |
...
<!-- count:tests -->1158<!-- /count --> bats tests across <!-- count:testFiles -->45<!-- /count --> files. A few anchors :
...
- <!-- count:tests -->1158<!-- /count --> bats tests run on every PR (Linux + macOS), parallelised via `./scripts/test.sh`
