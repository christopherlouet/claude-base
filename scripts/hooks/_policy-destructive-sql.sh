...
# Missing helper → no strip → possible over-block, never a bypass.
_pds_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd || true)
# shellcheck source=_core-helpers.sh
if [ -n "$_pds_dir" ] && [ -f "$_pds_dir/_core-helpers.sh" ]; then . "$_pds_dir/_core-helpers.sh"; fi
