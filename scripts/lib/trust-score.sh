...
trust_score() {
    ...
    forks=$(printf '%s' "$json" | jq -r '.forks_count // 0')
    pushed=$(printf '%s' "$json" | jq -r '.pushed_at // ""')
    archived=$(printf '%s' "$json" | jq -r '.archived // false')
    ...
    max_stale=$(jq -r '.global.maxStaleDays // 365' "$CURATION_THRESHOLDS")
    ...
    min_stars=$(jq -r ".tracks.${track}.minStars // 0" "$CURATION_THRESHOLDS")

    local age_days
    if [ -n "$pushed" ]; then
        age_days=$(curation_days_since "$pushed") || age_days=-1
    else
        age_days=-1
    fi

    # Verdict: collect ALL applicable reasons; fail dominates flag dominates pass.
    local verdict="pass"
    local reasons=()
    ...
    if [ "$age_days" -ge 0 ] && [ "$age_days" -gt "$max_stale" ]; then
        verdict="fail"; reasons+=("stale:${age_days}d>${max_stale}d")
    elif [ "$age_days" -lt 0 ]; then
        # Unknown recency is a soft concern, not a hard fail.
        [ "$verdict" = "pass" ] && verdict="flag"
        reasons+=("unknown-recency")
    fi
    ...
    case "$license" in
        ""|NONE|null)
            if [ "$verdict" != "fail" ]; then verdict="$missing_lic_verdict"; fi
