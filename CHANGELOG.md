...
says out loud actually happen?** The nightly curation bot printed `re-pin` in its digest for weeks
while re-pinning nothing. The README advertised blocks the hooks do not perform. `init --ci` promised
a working pipeline and installed the foundation's own, which goes red on the first push of any
project that is not this one. `update` reported success and left five-month-old security hooks in
place. Fourteen lifecycle hooks wrote to a world-readable path. None of these were found by reading
code: each is a claim that was taken to its real target and checked, and the fix is the one the
measurement pointed at.

### Added

- **Three stack-neutral CI templates for downstream projects** (`templates/github-workflows/`).
  `ci.yml` detects Node, Python and Go and runs only the tools the project actually has; `pr-check.yml`
  checks the PR title; `security.yml` runs Gitleaks from its pinned image, since the Gitleaks Action
  requires a licence key on organization-owned repositories. No stack and no collected tests are
  notices, not failures. Replayed with `act` on six fixture projects and in containers on `node:24`
  and `node:26` with npm, yarn classic, pnpm 8, pnpm 9 and pinned-manager lockfiles. No release
  template ships: a merge that finds release automation missing says so rather than reading as added.
  (#570)

- **`scripts/lib/pristine-hashes.txt`, the table that tells an untouched file from a customised one.**
  Generated by `scripts/gen-pristine-hashes.sh` from the full history — installs are depth-1 clones,
  so the history has to be carried rather than consulted. Re-derived on the day of this release:
  **96 hashes over 30 managed scripts**, covering every version of `scripts/hooks/*.sh` and
  `substance-check.sh` the foundation shipped across its 74 tags. The pre-commit hook regenerates and
  stages it when a managed script changes, and a bats self-application check fails CI when a shipped
  version is missing. (#574)

- **The digest names who holds the re-pin lock, and for how long.** The #458 lock stops a second
  re-pin PR while one is still open; it had no expiry and no voice. Draft #524 held it for 7 nights —
  measured in the bot's journal, findings climbing 9 → 12 — while every digest still printed `re-pin`
  as the action for each drift. The lock is now stated inline, escalates past
  `global.repinLockStaleDays` (default 3), lands in `digest.json` for machine consumers and repeats on
  stderr for the journal. An unknown age escalates instead of reading as "opened today", and `--dry-run`
  consults it too, since the lookup is a read. (#552)

- **The pin-time safety screen says where it matched.** Each flag now carries its findings —
  `{path, category, lineNumber, line}`, capped at 25 findings and 240 characters, with `findingsTotal`
  and `detailComplete`. The verdict never depends on that detail: whatever breaks the detail loses the
  detail, never the flag. (#568)

- **A Home Assistant skill for the `homelab` preset**, and the discovery source that can see it at all.
  `homeassistant-ai/skills` is recommended on evidence no model produced — 728 stars, pushed the day
  before, 74 files, MIT, and an angle (idiomatic configuration, native triggers over templates,
  `entity_id` over `device_id`) that fits a versioned configuration repository. The judge itself did
  **not** separate the two candidates over three draws each, and the ordering flipped against the
  single-draw run that preceded it, which is why it was sampled instead of believed. (#548, #551)

- **The curation bot's liveness can be watched from outside itself.** "Nightly" was a promise nobody
  checked: a bot that stops running raises nothing, and a failure notice sent by the bot cannot report
  that case. The wrapper writes the time of each successful run to the node_exporter textfile
  collector, and a new "Health alert" section gives the one-time root preparation, a Prometheus rule
  firing on a success older than 36 h **or** an absent series, and how to test it with promtool
  without `absent()` firing in the sampling gaps. (#575)

### Changed

- **A plain `update -y` now refreshes unmodified hook copies.** Until now a security fix reached an
  install only through the opt-in `--hook-scripts` flag, while a plain update stamped the new version
  and left every hook on its old rules behind a warning. Now that an untouched copy of an older
  release can be told from a customised one, the plain path replaces those too (maintainer decision),
  names each file it replaced, and adds transitively the `_*.sh` libraries the refreshed sources name
  when the old install lacks them — a hook written after a library split would otherwise source a
  missing file and fail closed. It never adds a new hook and never touches `settings.json`.
  Customised files still require `--force`. (#574)

- **The safety screen's executable-surface cap moves from 25 to 250 files.** The cap bounds API calls;
  it was never a safety judgement, and at 25 it froze the four largest vendor repositories in
  propose-only for good — their exec surface is 158, 142, 40 and 36 files of real per-skill code.
  Exceeding the cap still flags, whatever its value. The single reason that hid two different unknowns
  is split: GitHub truncating the tree (`exec-surface-truncated`, unbounded — files whose names we
  never saw) and passing our own cap (`exec-surface-over-cap`, files we named and chose not to fetch).
  (#564)

### Security

- **A home directory could be erased through `~` or `$HOME`.** The validator refused the literal
  `/home/<user>` path but read it literally, and the shell expands `~` and `$HOME` only after the guard
  has looked; the platform deny rule covers a single spelling and flag order. The home rule now also
  matches `~`, `~name`, `$HOME`, `${HOME}` and `${HOME:?}` — bare, with a trailing slash or a `/*`
  glob, in any flag order and argument position, and ending on `; & | )` as well as on whitespace. The
  command word must be a word of its own, so `terraform fmt ~` is no longer refused. Anything **inside**
  a home stays allowed. Measured before widening on 611 real agent deletion commands from transcripts:
  zero legitimate verdict changed. (#571)

- **Fourteen lifecycle hooks wrote payloads to world-readable `/tmp`.** Two copied the first 200 bytes
  of their payload — the refused command for `PermissionDenied`, the session id and transcript path for
  a permission notification — into `/tmp/claude-*.log`, readable by every account on the machine.
  `scripts/hooks/log-event.sh` now writes them to `~/.local/state/claude-base/` (`XDG_STATE_HOME`
  honoured), directory 700, files 600, appended under `umask 077`, recording a tag, a UTC timestamp and
  **named scalar fields only** (`tool_name`, `notification_type`, `reason`, `trigger`), each squeezed to
  one token, so no payload text can reach a log. A symlinked log path is skipped; no `HOME` means no
  log rather than a crash under `set -u`. The rewriter metric log, which holds the start of each
  command, moved with them. **Action required after upgrading: delete your existing
  `/tmp/claude-*.log` files — this release stops producing them, it cannot clean up the ones already
  written on your machine.** (#573)

- **A PR title was pasted into a workflow's `run:` body as shell.** `github.event.pull_request.title`
  was substituted as script source before the shell starts, so a crafted title executed on the runner
  and the job still went green. The title now reaches the shell through `env:`, in the foundation's own
  `pr-check.yml` and in the downstream template. The injection scanner that should have caught it was
  blind to a `}` inside the expression, to bracket access and to a continued plain scalar; it now sees
  all three. (#570)

- **The safety screen never read plugin hooks.** It treated `settings*.json` and `.mcp.json` as exec
  surface but not the plugin format's own declarations — `hooks/hooks.json`,
  `.claude-plugin/plugin.json`, `marketplace.json` — so a hostile plugin hook passed unread; 12 of the
  20 registry repositories ship such files. Reading them was not enough: real configs are
  pretty-printed and split the sink across lines (interpreter in `"command"`, payload in `"args"`),
  which a line-based scan cannot correlate. Each declared command is now also scanned as the single
  line it becomes when it runs. Verified: 25 plugin files actually read, and the 20 registry entries
  replay with identical verdicts. (#567)

- **Two evasions of the screen are closed.** In a UTF-8 locale, one invalid byte inside a
  download-piped-to-shell hid the line from every pattern (pre-existing); in the C locale,
  `[[:space:]]` missed Unicode spaces in a prompt injection. Every pattern now runs in both locales,
  the UTF-8 one pinned to any installed UTF-8 locale so it holds under cron. A grep error flags
  `scan-error`, an unrenderable verdict flags `screen-emit-failed` keeping every reason, and an
  unreadable screen output demotes the drift as `screen-output-invalid`. Exemptions for reviewed false
  positives were built and then dropped: a line exempted by its bytes can be made to execute by its
  surroundings. (#568)

### Fixed

- **`update --hook-scripts` left old security libraries in place.** `update` could not tell a hook
  script untouched since an older release from one the user customised — both differ from today's copy
  — so it skipped both, and `--force` discarded real customisations. Measured on a v5.3.0 install
  updated to 5.5.0: even `--hook-scripts` and `--all` kept `_policy-dangerous-commands.sh`,
  `_policy-secrets.sh`, `_policy-write-targets.sh`, `main-branch-guard.sh`, `setup-deps.sh` and
  `substance-check.sh` on their 5.3.0 rules while stamping 5.5.0. The same install now ends with zero
  stale scripts. The no-force replacement refuses to write through a symlink and refuses a downgrade
  from an older foundation checkout, and every replaced file is named rather than hidden behind
  `--verbose`. (#574)

- **npm's placeholder test blocked every commit.** `npm init -y` writes a test script that always
  exits 1; the pre-commit and pre-push gates ran it as a real suite, so a freshly initialised project
  could neither commit nor push before it had a single test. Both gates now recognise that exact script
  — exact match only, and only when there is no `pretest`/`posttest` — and skip it with a notice naming
  how to enable the gate. It means "no npm suite", not "no suite": pre-commit falls through to
  `pytest` and `go test`, so a Python or Go project carrying a `package.json` only for husky no longer
  commits over a red suite. (#569)

- **`init --ci` copied the foundation's own CI into every project.** `--ci`, `--all`, the six presets
  with `defaults.ci` and `--ci-existing merge|replace` all installed workflows that lint `./scripts`,
  run bats, run the counts gate and deploy a Docusaurus site — none of which a user's project has, so
  its first push went red. Installs now take the stack-neutral templates above. A merge that keeps the
  project's own `security.yml` says that no secret scan was added, and the docs no longer promise that
  existing workflows are never overwritten, which `--ci-existing replace` does. (#570)

- **A truncated `compare` re-pinned byte-identical subtrees.** GitHub caps `.files` at 300, and an
  all-outside verdict on a truncated list would be a false suppression, so the subpath drift check
  answered "unknown" and kept the drift — on a busy monorepo, forever. Measured on
  `anthropics/claude-code`: the range behind #562 returned exactly 300 files, none of them under the
  consumed `plugins/frontend-design`, whose tree is byte-identical across the two refs; four of that
  PR's nine entries re-pinned nothing while holding the lock. Where the file list says "unknown", the
  subtree's tree SHA is asked instead — the same question, put to an endpoint that can answer it.
  Resolution is deliberately narrow: only 40 hex characters count as an answer, because `gh api` prints
  a JSON error body on a bad ref and an emptiness check would compare two error bodies, find them
  equal and suppress a real drift. (#563)

- **A release that changed nothing consumed still re-pinned everything.** Three subpath-scoped
  repositories are tag-pinned, and one carries six records: 164 commits since June, 5 of them touching
  anything the foundation consumes, and three consecutive real release ranges touching zero consumed
  files. Each would have opened a six-record re-pin PR and blocked every other re-pin behind it. A tag
  range is now asked the same question as a sha range, with the same fail-safe — unknown stays drift.
  Root-scoped records are untouched: with no subpath there is no narrower question to ask. (#565)

- **A drift named every skill of a repository instead of the one that moved.** One repository can back
  several skills at their own subpaths; attribution was repo-level, and the reason recorded for it —
  the cost of a per-drift `compare` call — had expired, since the subpath check makes that exact call
  on the nightly path anyway. Measured on the two-record repository: 18 commits since June, 12 touching
  one subpath, 0 touching the other, and both skills named on every re-pin. The range is already
  fetched, so it now also answers **which** subpath moved. Narrowing is only ever done on an answer
  actually obtained; pinning stays per-repo-ref, so the lockstep invariant is untouched. (#566)

- **The licence probe only ever asked the repository root.** `anthropics/skills` sat in propose-only as
  `missing-license` while both consumed subpaths ship an Apache-2.0 `LICENSE.txt` — the repository has
  simply never had a root LICENSE. `trust_score` now consults each subpath's own licence file, after
  the root file and before the README, so no repository that passes today changes verdict or reason.
  **Every** subpath must carry one: waving a repository through on a licensed sibling would licence a
  skill that is not licensed. The delta was measured before widening, not after — exactly 2 of the 20
  registry records change verdict, and both are the ones that were blocked. (#554)

- **A judge that never answered was counted as a rejection.** A real discovery run reported "15
  candidates, 0 proposed, 15 rejected"; 9 were judged and 6 never answered. The engine already handles
  this class correctly twice — an unfetchable source says "coverage incomplete", an exhausted budget
  defers — only the judge turned an outage into a verdict, in the one field the digest carries. The
  fail-safe verdict now carries `unavailable: true`, such candidates are counted as `unjudged` (never
  rejected, never proposed), and the digest states in one line that these are not rejections.
  Deliberately not `deferred`: that word already means the budget stopped the run. (#549)

- **The judge scored candidates against a list the foundation had outgrown.** The fit rubric enumerates
  the domains a candidate is scored against; it named six and stopped at "testing" — no home
  automation, and no `self-hosted`, a module the foundation has shipped all along. Measured with one
  factor varied, three draws per arm: the subject moves 2, 2, 3 → 5, 4, 5 and crosses the threshold,
  while an unrelated control moves 1, 1, 1 → 2, 1, 2 and stays two points below it. The loosening is
  real and stated rather than hidden, which is why the fit threshold is **not** touched in the same
  change: if proposals turn noisy, the threshold is the knob, and it should move on its own evidence.
  (#550, and #548 for the discovery source that made home automation visible at all)

- **`work-brainstorm` pointed at a skill nothing can load.** `disable-model-invocation: true` does not
  only stop auto-triggering; it refuses a `Skill` call outright, including one made from inside the
  command that asks for it — measured, with a control that differs only in the flag and does load. The
  command carries its own process, so the pointer is replaced by the manual entry point. The guard that
  pins it scans the whole `.claude/` tree rather than `commands/` alone, matches the imperative form
  only (a "see also" line naming a manual-only skill is correct prose, not a defect) and asserts both
  halves have subjects, so it cannot pass vacuously. (#559)

- **The catalogue promised auto-trigger to 12 skills that cannot be triggered.** Re-derived today: 53
  skills, **12** of them model-disabled, five in the "Core skills" table — `work-commit`, `work-plan`,
  `work-explore`, `work-brainstorm`, `work-pr`, the rows most likely to be read first. The column is now
  "Trigger", those rows read **manual only** with the slash command that does start them, and the
  README's table and directory tree agree. The larger surface was generated: `generate-skill-docs.ts`
  emitted an "Automatic triggering" section on all 53 pages with no branch on the flag, advertising
  keywords for skills the harness refuses; it now branches, and the index header splits the total. The
  drift guard pins both directions on the real catalogue, with a negative probe that drives the real
  helper instead of re-implementing its regex beside it. (#560)

- **The website script tests existed and never ran.** `website/package.json` has defined `test:scripts`
  for as long as the four `scripts/*.test.ts` files have existed, and no workflow invoked it — 51 tests
  guarding nothing. That mattered here precisely: those files cover the doc generators, whose output
  under `website/docs/` is not versioned, so the counts gate's `git diff` cannot see it either, which is
  how #560's defect reached all 53 skill pages. Wired into `Lint & Counts`, which already installs the
  dependencies, for under a second. The stopgap source grep it replaces is removed rather than kept as
  a weaker second way to fail on the same fact. (#561)

### Documentation

- **The README promised blocks the hooks do not perform.** Every behavioural claim was checked by
  running the hook it names. The substance gate exits 0 on a hollow test — it flags, it does not block,
  while the README said the anti-gaming layer "blocks that"; "no network access at runtime" was false
  (the Setup hooks run `npm audit`, `npm outdated` and dependency installs, PostToolUse re-syncs
  dependencies on a manifest edit, and prompt-context calls `gh pr list` on every prompt); the secret
  gate covers seven high-confidence provider patterns through an edit, not other keys and not Bash
  redirects; no hook refuses a forced push, only the literal leading forms in the permission deny
  list; main-branch protection moves an edit to a new branch and refuses only when that branch cannot
  be created. `GUARDRAILS.md` marks the substance gate advisory and `POSITIONING.md` dates its June
  uniqueness claim against what the September re-audit found in peers' source. (#572)

- **Reading a secrets file is not guarded, and now says so.** The guards stop a key being written into
  a file and a secrets file being overwritten through Bash, but nothing stops `cat .env` or the `Read`
  tool, and what is read reaches the transcript. Blocking reads was weighed and declined — infrastructure
  work needs the agent to reach credentials — so the gap is stated instead of implied. (#572)

- **Model docs realigned on Fable 5.1 and the `fable` alias.** `claude-fable-5-1` replaced
  `claude-fable-5` as the default on 2026-09-01: same $10/$50 per MTok, but cache reads dropped from
  $1.00 to $0.25 against $0.50 on Opus 5 — so the "~2× Opus 5" shorthand repeated in four places
  overstates the gap precisely for long agentic sessions re-reading a cached prefix, which is what this
  foundation runs. A `fable` alias now exists in sub-agent `model:` frontmatter; three files said there
  was none and used it as part of the reason not to pin an agent to Fable. The recommendation is
  unchanged and now rests on cost, which is true, rather than on a tool limitation, which is not. Also
  corrected: the ARCHITECTURE table gave Sonnet 200k/64k where Sonnet 5 is 1M/128k, and
  advanced-features called Fable export-restricted on a page that dates the lifting of that directive.
  (#556)

- **The `/skill-doctor` section names its three columns before drawing a verdict.** The report moved
  into the `Stats` tab of the Plugins panel; it is not session-scoped (the history is persistent and
  counts days since last use), and its context figure is the skill's one-line listing charged every
  turn, not the cost of the skill body. The counters are machine-wide, not per-repository — settled by
  a control where five invocations credited to a skill were all typed in other projects on the same
  machine. So a "never invoked" row is evidence across the whole fleet, and the trap named in the
  section stands: a repository that authors a skill set rarely invokes it. (#558)

- Vendor pins advanced across the release, each with the safety screen re-run on the new ref and the
  range inspected for new scripts, exec bits, hooks and MCP config: #524, #553, #557, #562. The
  `anthropics/skills` re-pin is the one that mattered — the pin had sat on a 2026-07-07 ref for two
  months, and the `claude-api` skill it points at named neither `claude-opus-5` nor `claude-fable-5-1`.
  (#555)
