...
- **A false positive that demotes a skill at every re-pin is fixed by a reviewed exemption, never by loosening a pattern.** The screen's output names each finding (`path`, `category`, `lineNumber`, `line`, `lineSha256`; the first 25, with `findingsTotal`, and a line longer than 240 characters is cut and marked `lineTruncated` — its exemption needs the whole line, read from the file). After reading the line in context, record it in `.claude/curation/safety-exemptions.json` (with `reviewedRef`, `reviewedOn`, `rationale`) through a normal PR. An entry lifts only those exact line bytes in that file of that repo under that category (the recorded `line` must hash to its `lineSha256`, or the entry is ignored with a warning), so an edited or different line flags again; fail-safe reasons (unfetchable, truncated, over-cap, unresolved subpath) cannot be exempted. A re-pin that passes only through exemptions lists its lifted lines in its PR body, and says how many were left out when there are more than 25. Known limit: an identical copy of a reviewed line is lifted with it, whatever its context — the PR body lists every copy with its line number, so re-read the listed lines at the new ref before merging.
