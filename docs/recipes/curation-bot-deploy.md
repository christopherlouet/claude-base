...
- **A false positive that demotes a skill at every re-pin is fixed by a reviewed exemption, never by loosening a pattern.** The screen's output names each finding (`path`, `category`, `line`, `lineSha256`; the first 25, with `findingsTotal`, and a line longer than 240 characters is cut and marked `lineTruncated` — its exemption needs the whole line, read from the file). After reading the line in context, record it in `.claude/curation/safety-exemptions.json` (with `reviewedRef`, `reviewedOn`, `rationale`) through a normal PR. An entry lifts only that exact line in that file of that repo under that category, so an edited or new line flags again; fail-safe reasons (unfetchable, truncated, over-cap, unresolved subpath) cannot be exempted. A re-pin that passes only through exemptions lists its lifted lines in its PR body, and says how many were left out when there are more than 25. Known limit: an unchanged line whose surrounding context changes keeps its exemption — re-read the listed lines at the new ref before merging.
