...
| **Pre-commit tests** | PreToolUse (Bash git commit) | Runs the stack's test suite (npm/pytest/go) before a commit and **blocks (exit 2) on failure**; npm's `npm init -y` placeholder test script (always exits 1) is skipped with a notice, not run; detects/repairs Husky if needed. Disable with `SKIP_PRE_COMMIT_TESTS=1` (`scripts/hooks/pre-commit-tests.sh`) |
| **Local pre-push CI** | PreToolUse (Bash git push) | Lint + type-check + tests before push; npm's placeholder test script is skipped with a notice. Disable with `SKIP_PRE_PUSH_CI=1` |
