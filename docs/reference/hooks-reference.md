...
| **Pre-commit tests** | PreToolUse (Bash git commit) | Runs the stack's test suite (npm/pytest/go) before a commit and **blocks (exit 2) on failure**; detects/repairs Husky if needed. Disable with `SKIP_PRE_COMMIT_TESTS=1` (`scripts/hooks/pre-commit-tests.sh`) |
