...
The `sonnet` tier alias resolves to Sonnet 5 automatically, so agent `model: sonnet` frontmatter picks it up with **no change needed**. This foundation keeps **Opus 4.8 as the recommended default for complex/critical work** (TDD, Audit, architecture) — see [best-practices.md](./best-practices.md) — and uses Sonnet 5 where its price/perf wins: audits, analyses, and high-volume agentic passes.
